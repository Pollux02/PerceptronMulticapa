...
# Función para trazar los resultados de la clasificación
def plot_classification_results(datos, d, salidas):
    # Definir colores para cada clase
    colores = ['r', 'g', 'b', 'y', 'c', 'm']  # Puedes agregar más colores si necesitas

    # Graficar los puntos de datos
    for i in range(len(datos)):
        plt.scatter(datos[i, 0], datos[i, 1], color=colores[int(d[i])], marker='o')

    # Graficar las salidas predichas
    for i in range(len(datos)):
        plt.scatter(datos[i, 0], datos[i, 1], color=colores[int(salidas[i][0])], marker='x')
...
if "__main__"==__name__:
    datos = pd.read_csv('concentlite.csv') # leer archivo excel
    ...
    xj = (Datos_entrenamiento(matrix_data,x_inicio,x_n))
    ...
    plot_classification_results(xi, d, salidas)
